...
public class EnthusiaEventsFragment extends Fragment implements View.OnClickListener {
    ...
    private void setupEvent (View view, final int event) {
        getActivity().getActionBar().setTitle(EnthusiaEvents.events[event]);

        getActivity().findViewById(R.id.enthusia_events_details_event_image).setBackgroundResource(EnthusiaEvents.drawables[event]);
